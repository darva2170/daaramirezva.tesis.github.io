...
Recordar que en caso de no tener alguna libería instalada, se debe usar el comando install.packages("Nombre_librería"), esto debe realizarse una única vez.
...
En caso de que el data frame no haya considerado los respectivos factores como tal, es necesario hacer la transformación de las caterogrías. 
...
text(x=0.5,y=1.3,expression(("x+y+z+w")))
text(x=0.5,y=1.6,expression("(1)=X"))

text(x=2.5,y=1.3,expression(("x+y+z+w")))
text(x=2.5,y=1.6,expression("a=Y"))
...
text(x=0.5,y=4.3,expression(("x+y+z+w")))
text(x=0.5,y=4.6,expression("b=Z"))

text(x=2.5,y=4.3,expression(("x+y+z+w")))
text(x=2.5,y=4.6,expression("ab=W"))
...
bp1<- ggplot(datos_tension2, aes(x=Electrodo, y=Esfuerzo)) + 
      ...
      labs(title="Diagrama de cajas por\nElectrodo no consumible",x="Electrodo no consumible", y = "Resistencia última \na la tracción (MPa)")+
...
bp2<- ggplot(datos_tension2, aes(x=Mat_Aporte, y=Esfuerzo)) + 
      geom_boxplot(fill='#A4A4A4', color="black", width=0.5)+
      labs(title="Diagrama de cajas por\nmaterial de aporte",x="Aporte", y = "Resistencia última \na la tracción (MPa)")+
      scale_fill_grey() + theme_classic()
...
bp3 <- ggplot(datos_tension, aes(x=factor(Mat_Aporte),y=Esfuerzo)) + 
       ...
       scale_x_discrete(name = "Método de aporte",
                        labels=c("Aporte ER4043", "Sin material de aporte")) +
       ggtitle("Diagrama de cajas conjunto") +
       theme_bw()+
       theme(legend.position = "top")+
       scale_fill_brewer(palette = "Accent", name = "Electrodo\nno consumible", labels = c("Punto Verde", "Punto Violeta")) 


grid.arrange(arrangeGrob(bp1, bp2, nrow = 2),
             bp3,# Second row with 2 plots in 2 different columns
             ncol = 2) 
```
...
Se realiza Tukey, en caso de que así se requiera...
...
```{r}
prueba_var<-bartlett.test(modelo_lineal$residuals,datos_tension$Mat_Aporte)


ifelse(prueba_var$p.value<0.05, "Rechazo Ho", "No rechazo Ho")


leveneTest(datos_tension$Esfuerzo~datos_tension$Mat_Aporte*datos_tension$Electrodo)
